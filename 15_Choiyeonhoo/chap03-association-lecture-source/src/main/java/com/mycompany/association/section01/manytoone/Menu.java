...
public class Menu {
  ...
  /* @ManyToOne
  * - 다대일 관계 (N:1) 매핑에 사용
  * - Many(현재 Entity) To One(참조할 Entity)
  * - 여러 개의 메뉴가 하나의 카태고리에 속할 수 있는 관계
  * 
  * - cascade = CascadeType.PERSIST : 자식이 저장(persist)될 때 부모도 같이 저장(persist)
  *
  * - fetch = FetchType.EAGER (@ManyToOne 기본 값)
  *   - Menu Entity 조회 시, 바로 Category도 Joint 해서 같이 조회
  *
  * - fetch = FetchType.LAZY
  *  - Menu Entity 조회 시, 일단 Menu만 조회 (Category 조회 X)
  *  - Menu 내부 Category가 사용 되는 시점에 조회를 시작
  *
  * @JoinColumn
  * - Join을 위한 외래키(FK) 컬럼 매핑
  * */
