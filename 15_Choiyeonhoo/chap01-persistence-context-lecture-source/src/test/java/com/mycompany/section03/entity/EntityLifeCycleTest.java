package com.mycompany.section03.entity;
...
class EntityLifeCycleTest {
  ...
  void testTransient(int menuCode) {
    ...
    // foundMenu : EntityManager를 통해 DB에서 조회간 객체 -> 영속 상태(1차 캐시)
    ...
    // then
    assertNotSame(foundMenu,newMenu); // 두 객체의 주소가 다르면 성공
  ...
  void testManagedOtherEntityManager(int menuCode) {
    //when
    // lifeCycle.findMenuByMenuCode(menuCode) 호출 마다
  ...
  void testManagedSameEntityManager(int menuCode) {
    //given
    // EntityManager를 1개 얻어옴
    // -> 해당 EntityManager가 관리하는 persistence context가 별도로 존재
    EntityManager entityManager = EntityManagerGenerator.getInstance();

    //when
    // menuCode를 이용해서 조회
    // - entityManager를 이용해서 조회 -> 영속 상태
    // - 1차 캐시에 저장 -> 똑같은 menuCode를 통해 조회할 경우 재 사용됨
  ...
  void testDetachEntity(int menuCode, int menuPrice) {
    ...
    //    (Dirty Checking : @Entity 객체의 변화(DB와 SnapShot 차이)를 감지)
    
    // (현재)준영속 상태
    // -> DB 테이블과 매핑 X, 그냥 Java 객채의 값만 변경됨
    //    (Dirty Checking X)
  ...
  void testDetachAndMerge(int menuCode, int menuPrice) {
    ...
    // Dirty Checking의 대상이 아니여서 DB 값 수정 X
    foundMenu.setMenuPrice(menuPrice);
    ...
    entityManager.merge(foundMenu); // 준영속 -> 영속
                                    // Merge 되면서 foundMenu와 DB의 값이 다른걸 확인해서 변경함
    entityManager.flush(); // DB에 내용을 전달
  ...
  void testMergeUpdate(int menuCode, String menuName) {
    ...
    // 영속 객체를 반환 (Dirty Checking -> 변환된 부분 감지)
  ...
  void testMergeSave() {
    ...
    foundMenu.setMenuCode(999); // PK 수정 ( Menu.class에서
    //  @GeneratedValue(strategy = GenerationType.IDENTITY) // 기본키 생성 전략(AUTO_INCREMENT) 주석 처리했음 )

    entityManager.merge(foundMenu);
    // 1차 캐시와 준영속 객체를 병합하여, 영속 객체 반환
    // -> Dirty Checking -> menu_code(PK)가 999인 행이 없음을 확인
    // -> Insert 구문 생성
  ...
  void testRemoveEntity(int menuCode) {
    ...
    /* remove
     * 엔티티를 영속성 컨텍스트 및 데이터베이스에서 삭제한다. (Dirty Checking으로 비교해서 DB에 반영이 됨)
